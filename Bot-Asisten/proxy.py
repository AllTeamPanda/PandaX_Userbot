...
k =[
  ...
  [Button.url("My Source Code", "https://github.com/ilhammansiz/PandaX_Userbot")]
  ]


@Stark.on(events.NewMessage(pattern="^[!?/]proxy$"))
...
@Stark.on(events.CallbackQuery(data="https"))
async def https(event):
    us = (await Stark.get_me()).username
    caption = f"""
<b>‣Generated By - @{event.sender.username}</b>
<b>‣Proxy By - @{us} </b>
<b>© @TeamSquadUserbotSupport</b>
"""
...
@Stark.on(events.CallbackQuery(data="socks4"))
...
@Stark.on(events.CallbackQuery(data="socks5"))
