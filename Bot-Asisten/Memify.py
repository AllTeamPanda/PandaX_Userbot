...
from PandaX_Userbot import Stark as bot
from . import mansiez
borg = bot
...
async def _(event):
    ...
    if not reply_message.media:
        await event.reply("```Mohon Balas Ke Gambar/Sticker/Gif```")
        return
    reply_message.sender
    await bot.download_file(reply_message.media)
    if reply_message.sender.bot:
        await event.reply("```Balas ke pesan pengguna yang sebenarnya.```")
        return
    else:
        await event.reply(
...
async def draw_meme_text(image_path, text):
    ...
    m_font = ImageFont.truetype(
        "PandaVersion/Panda/KenFont.otf", int(
            (70 / 640) * i_width)
    )
    if ";" in text:
...
async def draw_meme_text(image_path, text):
    ...
    i_width, i_height = img.size
    m_font = ImageFont.truetype(
        "PandaVersion/Panda/default.ttf", int((95 / 730) * i_width)
    )
    if ";" in text:
        upper_text, lower_text = text.split(";")
    else:
