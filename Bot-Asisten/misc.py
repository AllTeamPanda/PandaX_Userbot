...
‣ `/alive` - Alive Panda.
‣ `/ping` - Ping bot.
‣ `/tr` - contoh /tr (code language) reply
"""
...
async def info(event):
    ...
    sed = await Stark(GetUserPhotosRequest(user_id=event.sender_id, offset=42, max_id=0, limit=80))
    ...
    sedd = await Stark(GetUserPhotosRequest(user_id=input_str, offset=42, max_id=0, limit=80))
...
async def _(event):
    await event.edit(MISC_HELP, buttons=[[Button.inline("« Bᴀᴄᴋ", data="helpbot")]])

@Stark.on(events.callbackquery.CallbackQuery(data="miscs"))
async def _(event):
    await event.edit(MISC_HELP, buttons=[[Button.inline("« Bᴀᴄᴋ", data="helpbott")]])
