...
from PandaX_Userbot import Stark as borg
from . import mansiez
tbot = borg
...
@mansiez(pattern="^/(set|see|del) var(?: |$)(.*)(?: |$)([\s\S]*)")
...
Heroku = """
- /logs melihat logs di heroku
- /usage daftar dyno 
"""

@tbot.on(events.callbackquery.CallbackQuery(data="heroku"))
async def _(event):
    await event.edit(Heroku, buttons=[[Button.inline("« Bᴀᴄᴋ", data="helpbot")]])

@tbot.on(events.callbackquery.CallbackQuery(data="herokus"))
async def _(event):
    await event.edit(Heroku, buttons=[[Button.inline("« Bᴀᴄᴋ", data="helpbott")]])
