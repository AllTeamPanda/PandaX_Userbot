...
async def autobot():
    ...
    if who.username:
        ...
        username = "Assist3n" + (str(who.id))[5:] + "Pbot"
    bf = "@BotFather"
    ...
    if isdone.startswith("Sorry,"):
        ...
        if nowdone.startswith("Done!"):
            ...
            await bot.send_message(bf, f"@{username}")
            await asyncio.sleep(1)
            await bot.send_file(bf, "Pand/Version/logo.jpg")
            await asyncio.sleep(3)
        ...
        await bot.send_message(bf, f"@{username}")
        await asyncio.sleep(1)
        await bot.send_file(bf, "Panda/Version/logo.jpg")
        await asyncio.sleep(3)
        await bot.send_message(bf, "/setabouttext")
        await asyncio.sleep(1)
        await bot.send_message(bf, f"@{username}")
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"Panda/modules/assistant/{shortname}.py")
        name = "Panda.modules.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
...
def remove_plugin(shortname):
    try:
        try:
            ...
            name = f"Panda.modules.{shortname}"
