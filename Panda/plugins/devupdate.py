    ...
    if HEROKU_API_KEY is not None:
        ...
        if HEROKU_APP_NAME is None:
            await event.edit(
                ...
                ' untuk dapat deploy perubahan terbaru dari PandaUserbot.`'
            )
            repo.__del__()
            return
        ...
        await event.edit('`[HEROKU]:'
                         '\nSedang MengUpdate PandaUserbot, Mohon Menunggu 5-7 Menit`'
                         )
        ups_rem.fetch(ac_br)
        ...
        if build.status == "failed":
            ...
            await event.edit("`PandaUserbot Berhasil Di Deploy!\n" "Restarting, Mohon Tunggu Sebentar.....`")
        ...
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID, "#BOT \n"
                "`PamdaUserbot Berhasil Di Update`")
...
async def update(event, repo, ups_rem, ac_br):
    ...
    await event.edit('**PandaUserbot** `Berhasil Di Update!`')
    await asyncio.sleep(1)
    await event.edit('**PandaUserbot** `Di Restart....`')
    await asyncio.sleep(1)
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, "#BOT \n"
            "**PandaUserbot Telah Di Perbarui.**")
...
async def upstream(event):
    ...
    if changelog == '' and force_update is False:
        await panda.edit(
            f'\nPandaUserbot Sudah Versi Terbaru\n')
        await asyncio.sleep(15)
        await event.delete()
        return repo.__del__()

    if conf is None and force_update is False:
        changelog_str = f'**Pembaruan Untuk PandaUserbot :\n\n💻 Pembaruan Data :**\n`{changelog}`'
