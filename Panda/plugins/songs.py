...
async def _(event):
    ...
    pandaevent = await edit_or_reply(event, "`wi8..! I am finding your song....`")
    ...
    try:
        ...
        pass
    stderr = (await _pandautils.runcmd(song_cmd))[1]
    if stderr:
        return await pandaevent.edit(f"**Error :** `{stderr}`")
    pandaname, stderr = (await _pandautils.runcmd(name_cmd))[:2]
    if stderr:
        return await pandaevent.edit(f"**Error :** `{stderr}`")
    # stderr = (await runcmd(thumb_cmd))[1]
    pandaname = os.path.splitext(pandaname)[0]
    # if stderr:
    #    return await pandaevent.edit(f"**Error :** `{stderr}`")
    song_file = Path(f"{pandaname}.mp3")
    if not os.path.exists(song_file):
        return await pandaevent.edit(
            f"Sorry!. I can't find any related video/audio for `{query}`"
        )
    await pandaevent.edit("`yeah..! i found something wi8..🥰`")
    pandathumb = Path(f"{pandaname}.jpg")
    if not os.path.exists(catthumb):
        pandathumb = Path(f"{pandaname}.webp")
    elif not os.path.exists(pandathumb):
        pass
    ...
    for files in (pandathumb, song_file):
...
async def _(event):
    ...
    stderr = (await _pandautils.runcmd(video_cmd))[1]
    if stderr:
        return await pandaevent.edit(f"**Error :** `{stderr}`")
    pandaname, stderr = (await _catutils.runcmd(name_cmd))[:2]
    if stderr:
        return await pandaevent.edit(f"**Error :** `{stderr}`")
    # stderr = (await runcmd(thumb_cmd))[1]
    try:
    ...
    # if stderr:
    #    return await pandaevent.edit(f"**Error :** `{stderr}`")
    pandaname = os.path.splitext(catname)[0]
    vsong_file = Path(f"{pandaname}.mp4")
    if not os.path.exists(vsong_file):
        vsong_file = Path(f"{pandaname}.mkv")
    ...
    await pandaevent.edit("`yeah..! i found something wi8..🥰`")
    pandathumb = Path(f"{pandaname}.jpg")
    if not os.path.exists(pandathumb):
        pandathumb = Path(f"{pandaname}.webp")
    elif not os.path.exists(pandathumb):
        pandathumb = None
    await event.client.send_file(
    ...
    await pandaevent.delete()
    for files in (pandathumb, vsong_file):
        if files and os.path.exists(files):
            os.remove(files)
...
async def shazamcmd(event):
    ...
    pandaevent = await edit_or_reply(event, "__Downloading the audio clip...__")
    try:
        ...
        LOGS.error(e)
        return await edit_delete(
            pandaevent, f"**Error while reverse searching song:**\n__{str(e)}__"
        )
...
async def _(event):
    ...
    pandaevent = await edit_or_reply(event, SONG_SEARCH_STRING, parse_mode="html")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            if baka[0].message.startswith(
                ...
                return await edit_delete(
                    pandaevent, SONG_NOT_FOUND, parse_mode="html", time=5
                )
            await pandaevent.edit(SONG_SENDING_STRING, parse_mode="html")
            ...
            return await pandaevent.edit(SONGBOT_BLOCKED_STRING, parse_mode="html")
        await event.client.send_file(
            event.chat_id,
            music,
...
async def _(event):
    ...
    chat = "@auddbot"
    pandaevent = await edit_or_reply(event, "```Identifying the song```")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await pandaevent.edit("Wait just a sec...")
            result = await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
        except YouBlockedUserError:
            await pandaevent.edit("```Please unblock (@auddbot) and try again```")
            return
    namem = f"**Song Name : **`{result.text.splitlines()[0]}`\
        \n\n**Details : **__{result.text.splitlines()[2]}__"
    await pandaevent.edit(namem)
