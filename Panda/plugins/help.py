...
async def help(event):
    ...
    if args:
        if args in PLG_INFO:
            await edit_or_reply(event, str(PLG_INFO[args]))
        else:
        ...
        await edit_or_reply(
            event,
            f"**✦ Daftar Perintah Untuk Panda-Userbot**\n"
            f"**✦ Jumlah** `{len(PLG_INFO[category])}` **Modules**\n"
            f"**✦ Owner:** [{user.first_name}](tg://user?id={user.id})\n\n"
            f"📚  {string}"
            f"\n\nSupport @TeamSquadUserbotSupport",
        )
        await event.reply(
            f"\n**Contoh Ketik** `{tr}help afk` **Untuk Melihat Informasi Module**"
        )
