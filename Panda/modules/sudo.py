...
from .. import CMD_HELP, HEROKU_API_KEY, HEROKU_APP_NAME, SUDO_HANDLER, SUDO_USERS
...
@pandacute(pattern="addsudo(?:\s|$)([\s\S]*)")
async def add(event):
    suu = event.text[9:]
...
@pandacute(pattern="delsudo(?:\s|$)([\s\S]*)")
async def _(event):
    if event.sender_id in SUDO_USERS:
        return
    suu = event.text[8:]
