...
# From pm and tagged messages logger for catuserbot by @mrconfused (@sandy1709)
# Recode ilhammansiz
# PandaUserbot
...
@PandaBot.ilhammansiz_cmd(incoming=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@PandaBot.ilhammansiz_cmd(incoming=True, func=lambda e: e.mentioned, edited=False, forword=None)
...
async def set_grplog(event):
    ...
    if Config.PM_LOGGER_GROUP_ID == -100:
        return await edit_delete(
            event,
            "__For functioning of this you need to set PRIVATE_GROUP_BOT_API_ID in config vars__",
            10,
        )
    input_str = event.pattern_match.group(1)
    if input_str == "off":
