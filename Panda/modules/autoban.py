...
from ..modules.sqldb import autoban_db, dnd_db
from  ..utils.linked import get_update_linked_chat
from telethon import events
from telethon.tl.types import Channel

from . import LOGS, PandaBot, ilhammansiz_cmd, edit_or_reply
from PandaXBahasa import get_string
from ..utils.tools import inline_mention
...
@ilhammansiz_cmd(
    pattern="autokick(?: |$)(.*)",
    command=("autokick", plugin_category),
    info={
        "header": "** Automatically kick new joined users from the group.",
        "usage": ["`{tr}autokick` <on/off>"],
    },
)
async def _(event):
    ...
    if match == "on":
        if dnd_db.chat_in_dnd(event.chat_id):
            return await edit_or_reply(event, "`Chat already in do not disturb mode.`", time=3)
        dnd_db.add_dnd(event.chat_id)
        event.client.add_handler(
            dnd_func, events.ChatAction(func=lambda x: x.user_joined)
        )
        await edit_or_reply(event, "`Do not disturb mode activated for this chat.`", time=3)
    elif match == "off":
        if not dnd_db.chat_in_dnd(event.chat_id):
            return await edit_or_reply(event, "`Chat is not in do not disturb mode.`", time=3)
        dnd_db.del_dnd(event.chat_id)
        await edit_or_reply(event, "`Do not disturb mode deactivated for this chat.`", time=3)


@ilhammansiz_cmd(
    pattern="cban$",
    command=("cban", plugin_category),
    info={
...
async def ban_cha(ult):
    if autoban_db.is_autoban_enabled(ult.chat_id):
        ...
        return await edit_or_reply(ult, "`Disabled Auto ChannelBan...`")
    if not (
        ult.chat.creator
        or (ult.chat.admin_rights.delete_messages or ult.chat.admin_rights.ban_users)
    ):
        return await edit_or_reply(ult,
    ...
    await edit_or_reply(ult, "`Enabled Auto ChannelBan Successfully!`")
...
async def do_magic(event):
    match = event.pattern_match.group(1)
    msg = await edit_or_reply(event, get_string("com_1"))
    if match == "list":
