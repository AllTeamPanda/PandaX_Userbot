...
from .. import CMD_HELP, COUNT_PM, LASTMSG, LOGS, PM_AUTO_BAN, PM_LIMIT, bot
from ..events import Cutepanda
from ..misc import edit_delete, edit_or_reply
...
async def permitpm(event):
    ...
    if (
        ...
        try:
            ...
            from ..modules.sql_helper.pm_permit_sql import is_approved
...
async def auto_accept(event):
    ...
    if (
        ...
        try:
            from ..modules.sql_helper.globals import gvarstatus
            from ..modules.sql_helper.pm_permit_sql import approve, is_approved
        except AttributeError:
            return
...
@bot.on(Cutepanda(outgoing=True, pattern=r"notifoff$"))
async def notifoff(noff_event):
    """For .notifoff command, stop getting notifications from unapproved PMs."""
    try:
        from ..modules.sql_helper.globals import addgvar
    except AttributeError:
...
@bot.on(Cutepanda(outgoing=True, pattern=r"notifon$"))
async def notifon(non_event):
    """For .notifoff command, get notifications from unapproved PMs."""
    try:
        from ..modules.sql_helper.globals import delgvar
    except AttributeError:
...
@bot.on(Cutepanda(outgoing=True, pattern=r"(?:setuju|ok)\s?(.)?"))
async def approvepm(apprvpm):
    """For .ok command, give someone the permissions to PM you."""
    try:
        from ..modules.sql_helper.globals import gvarstatus
        from ..modules.sql_helper.pm_permit_sql import approve
    except AttributeError:
...
@bot.on(Cutepanda(outgoing=True, pattern=r"(?:byeee|nopm)\s?(.)?"))
async def disapprovepm(disapprvpm):
    try:
        from ..modules.sql_helper.pm_permit_sql import dissprove
    except BaseException:
...
@bot.on(Cutepanda(outgoing=True, pattern=r"block$"))
...
@bot.on(Cutepanda(outgoing=True, pattern=r"unblock$"))
...
@bot.on(Cutepanda(outgoing=True, pattern=r"(set|get|reset) pmpermit(?: |$)(\w*)"))
async def add_pmsg(cust_msg):
    ...
    try:
        from ..modules.sql_helper import globals as sql
    except AttributeError:
