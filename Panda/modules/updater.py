...
from .. import CMD_HELP, HEROKU_API_KEY, HEROKU_APP_NAME, UPSTREAM_REPO_URL
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            await event.edit(
                f"{txt}\n"
                "**Kredensial Heroku tidak valid untuk deploy PandaUserbot dyno.**"
            )
            return repo.__del__()
        try:
            from ..modules.sql_helper.globals import addgvar, delgvar
        ...
        await event.edit(
            "`PandaUserbot Berhasil Di Deploy! Userbot bisa di gunakan kembali.`"
        )
...
async def update(event, repo, ups_rem, ac_br):
    try:
        ...
        repo.git.reset("--hard", "FETCH_HEAD")
    await event.edit("`PandaUserbot Berhasil Diupdate! Userbot bisa di Gunakan Lagi.`")
...
@pandacute(pattern="update( now| deploy|$)")
...
async def upstream(event):
    ...
    if conf == "deploy":
        await event.edit("`[HEROKU]: Update Deploy PandaUserbot Sedang Dalam Proses...`")
        await deploy(event, repo, ups_rem, ac_br, txt)
        return

    if changelog == "" and not force_update:
        await event.edit("**🐼 PandaUserbot Sudah Versi Terbaru**")
...
CMD_HELP.update(
    {
        ...
        \n\n  ➕  **Syntax :** `{cmd}update`\
        \n  ➕  **Function : **Untuk Melihat Pembaruan Terbaru PandaUserbot.\
        \n\n  ➕  **Syntax :** `{cmd}update deploy`\
        \n  ➕  **Function : **Untuk MengUpdate Fitur Terbaru Dari PandaUserbot.\
    "
    }
)
