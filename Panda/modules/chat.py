...
from .. import CMD_HELP, owner
from ..misc import edit_delete, edit_or_reply, get_user_from_event, pandacute


@pandacute(pattern="userid$")
...
@pandacute(pattern="inviteall ?(.*)")
async def get_users(event):
    man_ = event.text[11:]
    chat_man = man_.lower()
    restricted = ["@TeamSquadUserbotSupport", "@TeamSquadUserbotSupport"]
    if chat_man in restricted:
        await edit_or_reply(event, "**Anda tidak dapat Mengundang Anggota dari sana.**")
        await event.client.send_message(
            -1001159103924, "**Maaf Telah Mencuri Member dari Sini.**"
        )
        return
    if not man_:
        return await edit_or_reply(
