...
from .. import ALIVE_EMOJI, ALIVE_LOGO, ALIVE_TEKS_CUSTOM, BOT_VER, CHANNEL
from .. import CMD_HANDLER as cmd
from .. import CMD_HELP, GROUP, StartTime
from ..misc import bash, edit_or_reply, pandacute
...
@pandacute(pattern="(?:alive|on)\s?(.)?")
async def amireallyalive(alive):
    user = await alive.client.get_me()
    uptime = await get_readable_time((time.time() - StartTime))
    output = (
        f"**[Panda Userbot](https://github.com/ilhammansiz/PandaX_Userbot) Telah Di Aktifkan.**\n\n"
       
        f"**Version-ALL :** `{BOT_VER}` \n\n"

        f"┌ **👤Master :** [{user.first_name}](tg://user?id={user.id}) \n"
        f"├ **🗂Modules :** `{len(modules)} Modules` \n"
        f"└ **🕰Uptime :** `{uptime}` \n\n"
        f"    **[𝗦𝘂𝗽𝗽𝗼𝗿𝘁](https://t.me/{GROUP})** | **[𝗖𝗵𝗮𝗻𝗻𝗲𝗹](https://t.me/{CHANNEL})** | **[𝗢𝘄𝗻𝗲𝗿](https://t.me/diemmmmmmmmmm)**"
    )
    if ALIVE_LOGO:
        try:
            logo = ALIVE_LOGO
