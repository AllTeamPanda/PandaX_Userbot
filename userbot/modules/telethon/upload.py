...
async def pandalst_of_files(path):
    files = []
...
def sortthings(contents, path):
    ...
    for file in contents:
        pandapath = os.path.join(path, file)
        if os.path.isfile(pandapath):
            pandasort.append(file)
    for file in contents:
        pandapath = os.path.join(path, file)
        if os.path.isdir(pandapath):
            pandasort.append(file)
    return pandasort
...
async def upload(path, event, udir_event, pandaflag=None):  # sourcery no-metrics
    pandaflag or False
    ...
    if os.path.isdir(path):
        ...
        for file in Files:
            pandapath = os.path.join(path, file)
            await upload(Path(pandapath), event, udir_event)
