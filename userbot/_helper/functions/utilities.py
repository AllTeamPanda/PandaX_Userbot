...
class Utilities(AioHttp):
    def HelpDex(self, page_number, loaded_modules, prefix):
        ...
        if len(twins) > rows:
            twins = twins[mod_page * rows : rows * (mod_page + 1)] + [
                (
                    InlineKeyboardButton(
                        text="❰ Prev",
                        callback_data="{}_prev({})".format(prefix, mod_page),
                    ),
                    InlineKeyboardButton(text="Back", callback_data=f"open-start-dex"),
                    InlineKeyboardButton(
                        text="Next ❱",
                        callback_data="{}_next({})".format(prefix, mod_page),
                    ),
                )
            ]
        return twins
    ...
    async def IsAdmin(self, m: Message):
        ...
        data = await self.get_chat_member(chat_id=m.chat.id, user_id=m.from_user.id)
