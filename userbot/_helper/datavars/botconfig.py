...
BOTDV = ["BOT_NAME", "BOT_USERNAME", "BOT_ID", "BOT_BIO", "BOT_PIC"]
...
class BotConfig(object):
    ...
    def BotUserName(self):
        ...
        return (
            self.getdv("BOT_USERNAME") or self.BOT_USERNAME or self.bot.username or None
        )

    def BotMention(self):
        """Get bot mention"""
        return (
            f"[{self.BotName()}](tg://user?id={self.BotId()})"
            if self.BotName() and self.BotId()
            else None
        )
