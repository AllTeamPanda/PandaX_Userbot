...
class Panda_GlobalCollection(BASE):
    __tablename__ = "panda_globalcollection"
    ...
    def __repr__(self):
        return "<Panda Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, Panda_GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


Panda_GlobalCollection.__table__.create(checkfirst=True)

PANDA_GLOBALCOLLECTION = threading.RLock()
...
def add_to_collectionlist(keywoard, contents):
    with PANDA_GLOBALCOLLECTION:
        keyword_items = Panda_GlobalCollection(keywoard, tuple(contents))
...
def rm_from_collectionlist(keywoard, contents):
    with PANDA_GLOBALCOLLECTION:
        keyword_items = SESSION.query(Panda_GlobalCollection).get(
...
def del_keyword_collectionlist(keywoard):
    with PANDA_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(Panda_GlobalCollection.keywoard)
            .filter(Panda_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(Panda_GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist():
    try:
        return SESSION.query(Panda_GlobalCollection).count()
    finally:
        SESSION.close()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(Panda_GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(Panda_GlobalCollection).all()
        for x in all_groups:
