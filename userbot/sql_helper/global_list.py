...
class PandaGloballist(BASE):
    __tablename__ = "pandagloballist"
    ...
    def __repr__(self):
        return "<Panda global values '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, PandaGloballist)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


PandaGloballist.__table__.create(checkfirst=True)

PANDAGLOBALLIST_INSERTION_LOCK = threading.RLock()
...
def add_to_list(keywoard, group_id):
    with PANDAGLOBALLIST_INSERTION_LOCK:
        broadcast_group = PandaGloballist(keywoard, str(group_id))
...
def rm_from_list(keywoard, group_id):
    with PANDAGLOBALLIST_INSERTION_LOCK:
        broadcast_group = SESSION.query(PandaGloballist).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_list(keywoard):
    with PANDAGLOBALLIST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(PandaGloballist.keywoard)
            .filter(PandaGloballist.keywoard == keywoard)
            .delete()
        )
...
def get_list_keywords():
    try:
        chats = SESSION.query(PandaGloballist.keywoard).distinct().all()
...
def num_list():
    try:
        return SESSION.query(PandaGloballist).count()
    finally:
        SESSION.close()
...
def num_list_keywords():
    try:
        return SESSION.query(func.count(distinct(PandaGloballist.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_lists():
    try:
        ...
        all_groups = SESSION.query(PandaGloballist).all()
        for x in all_groups:
