...
class PandaBroadcast(BASE):
    __tablename__ = "pandabroadcast"
    ...
    def __repr__(self):
        return "<Panda Broadcast channels '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, PandaBroadcast)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


PandaBroadcast.__table__.create(checkfirst=True)

PANDABROADCAST_INSERTION_LOCK = threading.RLock()
...
def add_to_broadcastlist(keywoard, group_id):
    with PANDABROADCAST_INSERTION_LOCK:
        broadcast_group = PandaBroadcast(keywoard, str(group_id))
...
def rm_from_broadcastlist(keywoard, group_id):
    with PANDABROADCAST_INSERTION_LOCK:
        broadcast_group = SESSION.query(PandaBroadcast).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_broadcastlist(keywoard):
    with PANDABROADCAST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(PandaBroadcast.keywoard)
            .filter(PandaBroadcast.keywoard == keywoard)
            .delete()
        )
...
def get_broadcastlist_chats():
    try:
        chats = SESSION.query(PandaBroadcast.keywoard).distinct().all()
...
def num_broadcastlist():
    try:
        return SESSION.query(PandaBroadcast).count()
    finally:
        SESSION.close()
...
def num_broadcastlist_chats():
    try:
        return SESSION.query(func.count(distinct(PandaBroadcast.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_broadcastlists():
    try:
        ...
        all_groups = SESSION.query(PandaBroadcast).all()
        for x in all_groups:
